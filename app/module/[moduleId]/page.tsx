...
export default function ModuleLanding({
  ...
  const { moduleId: rawId } = use(params);
  const moduleId = parseInt(rawId, 10);
  const mod = getModule(moduleId);
  if (!mod) return notFound();

  return <ModuleLandingInner moduleId={moduleId} />;
}

function ModuleLandingInner({ moduleId }: { moduleId: number }) {
  const mod = getModule(moduleId)!;
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  <div
                    className={cn(
                      "size-10 rounded-lg flex items-center justify-center text-sm font-mono font-bold shrink-0 transition-colors",
                      completed
                        ? "bg-cdi-green/20 text-cdi-green"
                        : "bg-muted text-muted-foreground group-hover:bg-cdi-green/10 group-hover:text-cdi-green"
                    )}
                  >
                    {completed ? (
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-foreground group-hover:text-cdi-green transition-colors">
                      {topic.title}
                    </h3>
                    <p className="text-sm text-muted-foreground mt-1">{topic.subtitle}</p>
                  </div>
                  <svg className="size-5 text-muted-foreground group-hover:text-cdi-green group-hover:translate-x-1 transition-all shrink-0 mt-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
